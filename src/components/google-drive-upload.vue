...
const onUploadFile = async () => {
  if (props.modelValue && props.modelValue.length) {
    ...
    const rs = await GDrive.UploadFiles(props.modelValue.map(x => x.file), { parents: props.parent })
    emit('onUploaded', rs)
...
<template>
  <van-uploader v-model="files" :multiple="multiple" :after-read="onChange">
  ...
  <div class="flex justify-center">
    ...
    <button v-else type="button" @click="onUploadFile"
      class="w-full items-center text-center content-center px-3 py-3 text-xs font-medium text-white bg-sky-500 dark:bg-sky-600">
      <span class="mr-2">{{ $t('files.uploadData') }}</span>
    </button>
  </div>
  <div v-if="isLoading"
