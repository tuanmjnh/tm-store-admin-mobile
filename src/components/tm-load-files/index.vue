  ...
  <div class="tm-load-files">
    ...
    <div class="col">
      <q-input v-if="isGoogleSheet" v-model="urlData" square outlined :dense="dense" :disable="isLoading" :loading="isLoading"
               :placeholder="labelUrl" debounce="500" @update:model-value="onGetDataUrl">
...
import { getSheets, getSheetValues, getIdFromUrl } from "../../utils/gapi"
...
export default defineComponent({
  ...
  emits: ['onStart', 'onFinish', 'update:model-value', 'onError'],
  setup(props, { emit }) {
    ...
    const onCreateColumns = (fistRows, type = 0) => {
      const rs = []
      for (let i = 0; i < fistRows.length; i++) {
        if (type === 0) rs.push({ name: fistRows[i], field: i, label: fistRows[i], align: 'left', sortable: true })
        else if (type === 3) rs.push({ name: fistRows[i], field: fistRows[i], label: fistRows[i], align: 'left', sortable: true })
        else rs.push({ name: i, field: i, label: i, align: 'left', sortable: true })
      }
      return rs
    }
    const onCreateRows = (rows, type = 0) => {
      const rs = []
      if (type === 0) {
    ...
    // if (props.defaultUrl) {
    //   onGetDataUrl(props.defaultUrl)
    // }
    // Check modelValue null Reset
    watch(() => props.modelValue, (state, prevState) => {
      if (!state) onReset()
    }, { deep: true, immediate: true })
    // Check change type input Reset
    watch(() => isGoogleSheet, (state, prevState) => {
      onReset()
    }, { deep: true, immediate: true })

    return {
      isGoogleSheet, urlData, isLoading, file, sheets, sheet, range, rows, columns, onGetDataUrl,
      openFileDialog() {
        ...
        input.onchange = _ => {
          ...
          reader.onload = (e) => {
            ...
            workbook.value = read(e.target.result, { type: 'array' })
      ...
      onGetDataSheet(val) {
        ...
        if (isGoogleSheet.value) {
          getSheetValues({
            ...
            emit('onFinish', { rows: rows.value, columns: columns.value })
          ...
          else if (options.header === 0) options.header = 1
          const x = utils.sheet_to_json(worksheet, options)
          if (x && x.length && x[0].length) {
            if (props.options.header === 0) {
              columns.value = onCreateColumns(x[0], props.options.header)
              rows.value = onCreateRows(x, props.options.header)
